...
In the previous lab we performed a minimal installation of Bacula on our \texttt{backup} servers so that we could try out a backup/restore process. In this lab we're going to start preparing our Puppet modules to manage a complete Bacula installation across our systems. We will create three modules in this lab:

\begin{itemize}
	\item \texttt{bacula-director} will install and manage the Bacula Director service.  We will apply this module to our \texttt{backups} server.
	...
	\item \texttt{bacula-storage} will install and manage the Bacula Storage Daemon.  We will apply this module to our \texttt{backups} server.
\end{itemize}
...
\begin{itemize}
	\item The \texttt{bacula-director} module will ensure that the \texttt{bacula-director} service is running on \texttt{backups}
	\item The \texttt{bacula-storage} module will ensure that the \texttt{bacula-sd} module is running on \texttt{backups}
...
Once these modules are prepared and applied we can begin modifying the configurations to support our backup and restore jobs. We will do this next week.
\end{document}
