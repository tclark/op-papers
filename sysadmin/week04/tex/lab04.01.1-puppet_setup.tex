...
Restart puppetmaster with the command \texttt{sudo service puppetmaster restart}.
...
\begin{verbatim}
sudo puppet agent --server=mgmt.foo.org.nz --no-daemonize --verbose
\end{verbatim}
...
Now, on the puppetmaster, run the command \texttt{sudo puppet cert --list}.  You should see the signing request for your agent.  Sign the key with the command \texttt{ sudo puppet cert --sign db}.
